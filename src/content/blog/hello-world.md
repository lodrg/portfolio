...
对于算法这块一直是常看常新的状态，每过一段时间感受就不一样了  
...
两个链表的合并，需要第三个链表去一点一点的获得两个链表的节点。
这个过程是比较清晰的，只要维护好指针的变化和两个链表的大小关系的判断就可以了。

问题出在长链表的第一个节点，因为是链表，新建一个链表的时候其实就是新建第一个节点。
问题是新建第一个节点的时机，我们还不知道第一个节点里面的值是什么。

这时候就需要一个占位的节点，他的值是是什么不重要，重要的是所有的事情完成以后，需要把返回的链表去掉头节点

dummy 就是用来解决新链表的首节点的这种问题的   
